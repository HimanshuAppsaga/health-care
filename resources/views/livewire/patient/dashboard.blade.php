<div class="p-8 max-w-4xl mx-auto font-manrope">
    <!-- Main Queue Card -->
    <div class="bg-surface rounded-[2rem] shadow-clinical border border-outline-variant overflow-hidden">
        <!-- Card Header -->
        <div class="p-8 border-b border-outline-variant flex items-center bg-surface-container-low">
        ...
        <!-- Card Body -->
        <div class="p-16 flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <div class="relative mb-6 z-10">
                <div class="text-9xl font-black text-[#005bb0] tracking-tighter leading-none select-none">
                    {{ $nowServing ? $nowServing->token_number : '— —' }}
                </div>
            </div>

            <h4 class="text-2xl font-bold text-on-background mb-10 relative z-10">
                {{ $nowServing ? ($nowServing->appointment->name ?? 'Unknown Patient') : 'No Patient Assigned' }}
            </h4>

            <div class="flex items-center justify-center gap-3 relative z-10">
                <p class="text-sm font-bold text-outline uppercase tracking-widest">Next Tokens:</p>
                @forelse($nextTokens as $token)
                    <span class="px-5 py-2 bg-surface-container text-[#005bb0] font-black rounded-full text-base border border-outline-variant/30 transition-all hover:scale-105">
                        {{ $token->token_number }}
                    </span>
                @empty
                    <span class="px-5 py-2 bg-surface-container text-outline font-black rounded-full text-base border border-outline-variant/30">
                        None
                    </span>
                @endforelse
            </div>
    ...
    @if($appointment)
        <div class="mt-8 bg-primary-container/10 rounded-[2rem] border border-primary/20 p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
            <div class="flex items-center gap-5">
                <div class="w-16 h-16 bg-[#005bb0] text-white rounded-2xl flex items-center justify-center font-black text-2xl shadow-lg shadow-[#005bb0]/25">
                    {{ $appointment->token ?? '—' }}
                </div>
                <div>
